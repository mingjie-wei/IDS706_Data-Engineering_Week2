...
# Correct Data Types
df["Purchase_Amount"] = (
    df["Purchase_Amount"].str.replace("$", "", regex=False).astype(float)
)
...
rfm_df["RF_Segment"] = rfm_df["R_Score"].astype(str) + rfm_df["F_Score"].astype(str)
...
features_df["Customer_Loyalty_Program_Member"] = features_df[
...
print("Training target distribution:", pd.Series(y_train).value_counts(normalize=True))
print("Testing target distribution:", pd.Series(y_test).value_counts(normalize=True))
...
plt.plot(fpr, tpr, label=f"XGBoost (AUC = {xgb_auc:.3f})", color="darkgreen", lw=2)
